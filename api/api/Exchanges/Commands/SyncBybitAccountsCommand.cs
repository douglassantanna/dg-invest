...
public class SyncBybitAccountsCommandHandler : IRequestHandler<SyncBybitAccountsCommand, Response>
{
    ...
    public async Task<Response> Handle(SyncBybitAccountsCommand request, CancellationToken cancellationToken)
    {
        ...
        var existingTags = user.Accounts.Select(a => a.SubaccountTag.ToLowerInvariant()).ToHashSet();
        ...
        {
            var tag = member.Username.Trim();
            if (existingTags.Contains(tag.ToLowerInvariant()))
            {
                matched++;
                _logger.LogInformation("SyncBybitAccounts: sub-account '{Tag}' already mapped for user {UserId}", tag, request.UserId);
                continue;
            }
            ...
            created++;
            _logger.LogInformation("SyncBybitAccounts: created account '{Tag}' for user {UserId}", tag, request.UserId);
        }
